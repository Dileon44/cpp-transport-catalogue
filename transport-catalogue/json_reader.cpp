...
namespace catalogue {
	...
	void JSONReader::ReadJSON(std::istream& input) {
		...
		if (input_doc.GetRoot().IsMap()
			...
			base_requests_ = json::Document{
				input_doc.GetRoot().AsMap().at("base_requests")
			};
	...
	void JSONReader::AddDistanceBetweenStops(const json::Node& stop_from) {
		for (const auto& stop_to : stop_from.AsMap().at("road_distances").AsMap()) {
			catalogue_.SetDistance(
				stop_from.AsMap().at("name").AsString(),
				stop_to.first,
	...
	void JSONReader::AddJsonBus(const json::Node& node) {
		...
		bus.type_route = node.AsMap().at("is_roundtrip").AsBool()
					   ? TypeRoute::CIRCLE
					   : TypeRoute::DIRECT;
	...
	std::map<std::string, json::Node> JSONReader::GenerateAnswerAboutRoute(const json::Node& request) const {
		...
		if (catalogue_.FindBus(bus_name)) {
			bus_info.insert({ "curvature", json::Node{
			request_handler_.GetBusStat(bus_name)->curvature } }
			);
			bus_info.insert({ "route_length", json::Node{
				request_handler_.GetBusStat(bus_name)->length_route } }
			);
			bus_info.insert({ "stop_count", json::Node{
				static_cast<int>(request_handler_.GetBusStat(bus_name)->number_stops) } }
			);
			bus_info.insert({ "unique_stop_count", json::Node{
				static_cast<int>(request_handler_.GetBusStat(bus_name)->unique_stops) } }
			);
		}
		else {
